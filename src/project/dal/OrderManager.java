...
class FixEngine extends quickfix.fix42.MessageCracker implements
	...
	public void onMessage(quickfix.fix42.ExecutionReport message,
			...
			IncorrectTagValue {
		/*
		 * Iterate through all fields in fix message and stored to DB? We wind
		 * up doing this in the OrderBroker ...
		 * 
		 * Iterator fieldIterator = message.iterator(); while
		 * (fieldIterator.hasNext()) { Field field =
		 * (Field)fieldIterator.next(); String tag =
		 * Integer.toString(field.getTag()); String value =
		 * message.getString(field.getTag()); }
		 */
	...
	public void sendNewOrder(boolean buy, String stock, double px, int shares) {
		ClOrdID orderId = new ClOrdID("EQUITY-TRADER-"
				+ System.currentTimeMillis());
		Side s = null;
		HandlInst handInst = new HandlInst(
		...
		try {
			...
			Logger log = Logger.getLogger("DATA ACCESS LAYER:");
			log.error("ERROR - session not found " + e);
		}
	}
}
...
class QuickFixConfigBuilder {
	public QuickFixConfigBuilder() {
		...
		fileStoreSettings.setString(FileStoreFactory.SETTING_FILE_STORE_PATH,
				"fixMsgStore");
		...
		sessionDict.setString(Initiator.SETTING_SOCKET_CONNECT_HOST,
				"127.0.0.1");
